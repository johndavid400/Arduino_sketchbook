...
// this sketch counts up from 0:00 to 59:59 (59 minutes and 59 seconds), then resets to 0:00 and adds a decimal each hour for up to 4
//hours.
...
int shift2_off = B11010000;
int shift2_blink_off = B11110000;
 ...
 */
int digit1_blink_on = B11100001;
int digit2_blink_on = B11100010;
int digit3_blink_on = B11100100;
int digit4_blink_on = B11101000;

int digit_off = B11000000;


// set the initial time here (it is set to 12:30)
int d1 = 0;
int display1;
int d2 = 3;
int display2;
...
void loop() {
  /*
  if (millis() > timer + 500 && millis() < timer + 502) {
   digitalToggle(blink_pin); 
   }
   */
  if (millis() >= timer + 999){
    timer = millis();
  ...
  if (blink_state == 1){
    shiftOut(dataPin, clockPin, MSBFIRST, digit1_blink_on); 
  }
  else{
    shiftOut(dataPin, clockPin, MSBFIRST, digit1_on); // send out 2nd set of serial data (B00000000 = all 8 pins LOW) to 1st shift register
  }
  shiftOut(dataPin, clockPin, MSBFIRST, display1); // send out 1st set of data, pushing 2nd set into 2nd shift register.
  digitalWrite(latchPin, HIGH);
  delay(time);
  digitalWrite(latchPin, LOW);
  if (blink_state == 1){
    shiftOut(dataPin, clockPin, MSBFIRST, shift2_blink_off); 
  }
  else{
  ...
  digitalWrite(latchPin, LOW);
  if (blink_state == 1){
    shiftOut(dataPin, clockPin, MSBFIRST, digit2_blink_on); 
  }
  else{
  ...
  digitalWrite(latchPin, LOW);
  if (blink_state == 1){
    shiftOut(dataPin, clockPin, MSBFIRST, digit3_blink_on); 
  }
  else{
  ...
  digitalWrite(latchPin, LOW);
  if (blink_state == 1){
    shiftOut(dataPin, clockPin, MSBFIRST, digit4_blink_on); 
  }
  else{
